...
function App(derby, name, filename) {
  ...
  this.model = null;
  this.page = null;
  this.serverRender = false // TODO: make this a parameter in future
  this._init();
}
...
App.prototype._loadBaseViews = function() {
  ...
  this.views.register('BootstrapPage',
    ...
    '<view is="{{$render.prefix}}Styles"></view>' +
    '<view is="{{$render.prefix}}Head"></view>' +
    '<body><div id="BootstrapLoading"></div>',
    {serverOnly: true}
  );
...
App.prototype._finishInit = function() {
  ...
  this.emit('load', page);
  // Emit global event on window that the app loaded
  setTimeout(function() {
    try {
      window.dispatchEvent(new CustomEvent('AppLoaded'))
    } catch (err) {
      console.warn('error emitting the AppLoaded event', err.stack)
    }
  }, 0)
};
